...
function Request(uri, options) {
  ...
  if (this.options.query && !this.url.query) {
    this.url.query = (typeof this.options.query == 'object') ? qs.stringify(this.options.query) : this.options.query;
  }
...
var parsers = {
  ...
  json: function (data, callback) {
    this._json(data, function (err, parsedData) {
      if (!err) return callback(null, parsedData); // all ok here
      // but here, we may have a security string prepended to the json data --
      // some supposedly restful apis are doing this because of a suppose'd security flaw in HATEOAS in the
      // insane belief that prepending javascript junk to the start of the 'json' response will fix this
      // and that developers won't realise how easy it is to just try again without the first line of the data
      // so, we're going to try again without the first line of the data
      var a = data.substring(data.indexOf("\n") + 1);
      this._json(a, callback);
    })
  },
  _json: function (data, callback) {
    if (data && data.length) {
      var parsedData;
      try {
...
function Service(defaults) {
  ...
  if (defaults.rateLimit) {
    defaults.rateLimit.isJson = false;
    defaults.rateLimit.isGet = false;
    defaults.rateLimit.isPost = false;
    defaults.rateLimit.isPut = false;
    defaults.rateLimit.isDel = false;
    defaults.rateLimit.isPatch = false;
    defaults.rateLimit.isRequest = false;

    if (!('methods' in defaults.rateLimit))
      defaults.rateLimit.methods = ['json', 'get', 'post', 'put', 'del', 'patch', 'request'];

    defaults.rateLimit.methods.forEach(function (method) {
      switch (method.toLowerCase()) {
        case 'json':
          defaults.rateLimit.isJson = true;
          defaults.rateLimit.json = limit(json).to(defaults.rateLimit.to).per(defaults.rateLimit.per).evenly(true);
          break;
        case 'get':
          defaults.rateLimit.isGet = true;
          defaults.rateLimit.get = limit(get).to(defaults.rateLimit.to).per(defaults.rateLimit.per).evenly(true);
          break;
        case 'post':
          defaults.rateLimit.isPost = true;
          defaults.rateLimit.post = limit(post).to(defaults.rateLimit.to).per(defaults.rateLimit.per).evenly(true);
          break;
        case 'put':
          defaults.rateLimit.isPut = true;
          defaults.rateLimit.put = limit(put).to(defaults.rateLimit.to).per(defaults.rateLimit.per).evenly(true);
          break;
        case 'del':
          defaults.rateLimit.isDel = true;
          defaults.rateLimit.del = limit(del).to(defaults.rateLimit.to).per(defaults.rateLimit.per).evenly(true);
          break;
        case 'patch':
          defaults.rateLimit.isPatch = true;
          defaults.rateLimit.patch = limit(patch).to(defaults.rateLimit.to).per(defaults.rateLimit.per).evenly(true);
          break;
        case 'request':
          defaults.rateLimit.isRequest = true;
          defaults.rateLimit.request = limit(request).to(defaults.rateLimit.to).per(defaults.rateLimit.per).evenly(true);
          break;
        default:
          throw method + ' is not a valid method -- please only use json, get, post, put, del, patch, and/or request';
      }
    });
  }
...
mixin(Service.prototype, {
  request: function (path, options) {
    if (this.defaults.rateLimit && this.defaults.rateLimit.isRequest) {
      return this.defaults.rateLimit.request(this._url(path), this._withDefaults(options));
    } else {
      return request(this._url(path), this._withDefaults(options));
    }
  },
  get: function (path, options) {
    if (this.defaults.rateLimit && this.defaults.rateLimit.isGet) {
      return this.defaults.rateLimit.get(this._url(path), this._withDefaults(options));
    } else {
      return get(this._url(path), this._withDefaults(options));
    }
  },
  patch: function (path, options) {
    if (this.defaults.rateLimit && this.defaults.rateLimit.isPatch) {
      return this.defaults.rateLimit.patch(this._url(path), this._withDefaults(options));
    } else {
      return patch(this._url(path), this._withDefaults(options));
    }
  },
  put: function (path, options) {
    if (this.defaults.rateLimit && this.defaults.rateLimit.isPut) {
      return this.defaults.rateLimit.put(this._url(path), this._withDefaults(options));
    } else {
      return put(this._url(path), this._withDefaults(options));
    }
  },
  post: function (path, options) {
    if (this.defaults.rateLimit && this.defaults.rateLimit.isPost) {
      return this.defaults.rateLimit.post(this._url(path), this._withDefaults(options));
    } else {
      return post(this._url(path), this._withDefaults(options));
    }
  },
  json: function (method, path, data, options) {
    if (this.defaults.rateLimit && this.defaults.rateLimit.isJson) {
      return this.defaults.rateLimit.json(this._url(path), data, this._withDefaults(options), method)
    } else {
      return json(this._url(path), data, this._withDefaults(options), method);
    }
  },
  del: function (path, options) {
    if (this.defaults.rateLimit && this.defaults.rateLimit.isDel) {
      return this.defaults.rateLimit.del(this._url(path), this._withDefaults(options));
    } else {
