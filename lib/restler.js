...
function Service(defaults) {
    ...
    if (this.defaults.rateLimit) {
        this.defaults.rateLimit.json = limit(this.json).to(this.defaults.rateLimit.to).per(this.defauls.rateLimit.per).evenly(true);
    }
...
mixin(Service.prototype, {
    ...
    json: function (path, data, options, method) {
        if (this.defaults.rateLimit) {
            this.defaults.rateLimit.json(this._url(url), data, this._withDefaults(options), method)
        } else {
            return json(this._url(url), data, this._withDefaults(options), method);
        }
    },
